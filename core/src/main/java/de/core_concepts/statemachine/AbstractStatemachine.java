...
import static de.core_concepts.statemachine.LifecycleEvent.SUCCESSFUL_STATE_CHANGE;
import static de.core_concepts.statemachine.LifecycleEvent.UNKNOWN_EVENT;
import static de.core_concepts.statemachine.LifecycleEvent.VALIDATION_ERROR;
    ...
    try {
      ...
      if (!transitionWhichPassesGuard.isPresent()) {
        ...
        if (!isReflexiveTransition(fromState, toState)) {
          ...
          getContext().setDataFor(SUCCESSFUL_STATE_CHANGE, toState.getId());
